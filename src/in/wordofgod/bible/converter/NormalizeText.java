...
public class NormalizeText {
	...
	private static String removeNumberCommas(String text) {
		...
		return text.replaceAll("(\\d),(\\d)", "$1$2").replaceAll("(\\d),(\\d)", "$1$2")
				.replaceAll("(\\d),(\\d)", "$1$2").replaceAll("(\\d),(\\d)", "$1$2").replaceAll("(\\d),(\\d)", "$1$2");
	}
	...
	private static String removeUnreadableCharacters(String text) {
		...
		// Remove non-breaking spaces and other problematic Unicode whitespace
		// characters
		...
		text = text.replace("‍", "");
		text = text.replace(" ", ""); // Hair space -> remove
		text = text.replace("", ""); // Single low-9 quotation mark -> remove
		text = text.replace("", ""); // Double low-9 quotation mark -> remove
		text = text.replace("", ""); // Double high-reversed-9 quotation mark -> remove
		text = text.replace(" ", ""); // Em space -> remove
		text = text.replace(" ", "");// Four-per-em space -> remove
		text = text.replaceAll("‌", "");// Zero width non-joiner -> remove
		text = text.replace("", "");// Six-per-em space -> remove
		return text;
	}
