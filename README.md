# bible-converter
Converts given Bible text in TheWord format to various output formats including JSON, text files, MS Word, MyBible.Zone SQLite3 databases, MySword SQLite3 databases, HTML, ThML, OSIS, Zefania XML, Microsoft Excel, and CSV.
...
- **CSV Support**: Generate four plain UTF-8 CSV files — `info`, `verses`, `books`, and `books_all` — ideal for data analysis, database import, and spreadsheet tools
...
### 8. **MSWord**
Converts the entire Bible into a **single Microsoft Word (.docx) document** with navigable structure — title page, book details, index, and full content with clickable hyperlinks between index and content.

- **Single output file** named `[ABBR].docx` (e.g., `TOV2017.docx`)
- **A4 portrait** page layout (595 × 842 pt)
- **Document metadata** — creator set to `Yesudas Solomon, www.WordOfGod.in`
- **Title page** — Bible common name (72pt), statistics (books / chapters / verses / words / unique words), publisher, and free-sharing notice
- **Book details page** — common name, short name, long name, long English name, translator, publisher, copyright, download links, contact, and social links
- **Navigation notice page** — guidance for mobile users
- **Index page** — clickable book list followed by a 3-column chapter index, all hyperlinked to their content sections
- **Content** — per book: heading page with chapter links (1-column), then all chapters and verses in a 2-column layout
  - Chapter headings with grey background shading
  - Verse number in bold, verse text in normal weight
  - Notes (`<n>…</n>`) rendered in grey (`#808080`)
  - Emphasis words (`<e>…</e>`) rendered in blue (`#3A7DCC`)
- **Console statistics** — total books, chapters, verses, words, and unique words printed on completion

```bash
java -jar bible-converter.jar MSWord /path/to/taOV.ont /path/to/taOV-information.ini
```

**Output structure:**
```
Output/MSWord/தமிழ்/TOV2017/
└── TOV2017.docx
```
...
```
[Title Page]
  Bible Common Name                  (72pt Cambria, centered)
  | 66 Books | | 1189 Chapters | …  (22pt Cambria, centered)
  By: Publisher Name                 (20pt Cambria, centered)
  "Freely Received; Freely Give…"   (18pt Calibri, centered)

[Book Details Page]
  First Edition 1986
  Common Name: ...  Short Name: ...  Long Name: ...
  Translated By: ... Published By: ...
  Created By: Yesudas Solomon, www.WordOfGod.in
  Copyright: ...
  Download / Contact / Follow links

[Navigation Notice Page]

[Index Page]
  Index                              (35pt, grey shading)
  Genesis  Exodus  Leviticus …      (19pt, clickable links)
  --- 3-column chapter index ---
  Genesis : 1  2  3 …  Exodus : 1  2 …

[Content — per book]
  Genesis                            (36pt Cambria, centered)
  1  2  3 … (chapter links)
  Genesis 1                          (18pt, grey shading, bookmark)
  1. In the beginning…               (bold number, normal text, 14pt)
  2. And the earth…
  …
```
...
### 19. **CSV**
Converts Bible text to **four separate UTF-8 CSV files** — one per data type — ideal for data analysis, importing into databases, or opening in any spreadsheet tool. All string fields are RFC 4180-quoted (double-quotes escaped as `""`) so Unicode text (Tamil, Hebrew, etc.) and embedded commas or newlines are handled safely.

- Output files are named `[ABBR]_info.csv`, `[ABBR]_verses.csv`, `[ABBR]_books.csv`, `[ABBR]_books_all.csv`
- Encoding: **UTF-8**
- All text fields are always double-quoted per RFC 4180

```bash
java -jar bible-converter.jar CSV /path/to/taOV.ont /path/to/taOV-information.ini
```

**Output structure:**
```
Output/CSV/தமிழ்/TRHE1836/
├── TRHE1836_info.csv
├── TRHE1836_verses.csv
├── TRHE1836_books.csv
└── TRHE1836_books_all.csv
...
**File 1 — `[ABBR]_info.csv`**

Key/value metadata about the Bible translation.

| Column | Description |
|---|---|
| Property | Metadata field name |
| Value | Metadata field value |

Rows include: Abbreviation, Common Name, Short Name, Long Name, Long English Name, Language Code, Published Year, Published By, Translated By, Copyright, Additional Information, Has OT, Has NT, Total Books, Total Chapters, Total Verses, Total Words, Total Unique Words, Generated Date, Generated By.

**File 2 — `[ABBR]_verses.csv`**
...
**File 3 — `[ABBR]_books.csv`**

One row per book that is present in this Bible.
...
**File 4 — `[ABBR]_books_all.csv`**

All 66 canonical books (Genesis–Revelation), regardless of whether they are present in this Bible.

| Column | Description |
|---|---|
| Book No | Sequential book number (1–66) |
| OSIS ID | Canonical OSIS book identifier |
| Book Name | Localised name if present in this Bible, English name otherwise |
| Testament | `OT` or `NT` |
| Present | `Yes` if in this Bible, `No` if absent |
| Chapters | Number of chapters (0 if absent) |
